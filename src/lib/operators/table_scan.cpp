...
std::shared_ptr<const Table> TableScan::_on_execute() {
  ...
  auto impl = make_unique_by_column_type<BaseTableScanImpl, TableScanImpl>(column_type, input_table, _column_id,
...
template <typename T>
TableScan::TableScanImpl<T>::TableScanImpl(std::shared_ptr<const Table> table, ColumnID column_id,
                                           const ScanType scan_type, const AllTypeVariant search_value)
    : _table(table), _column_id(column_id), _scan_type(scan_type), _search_value(search_value) {}

template <typename T>
void TableScan::TableScanImpl<T>::_scanColumn(std::shared_ptr<PosList> position_list,
                                              std::shared_ptr<ValueColumn<T>> vc, const ChunkID chunk_id) {
  const auto& values = vc->values();
  const auto compare_value = opossum::type_cast<T>(_search_value);

  const auto accessor = [&](size_t chunk_offset) { return values[chunk_offset]; };
  _appendPositionList(position_list, accessor, values.size(), compare_value, chunk_id, false);
}

template <typename T>
void TableScan::TableScanImpl<T>::_scanColumn(std::shared_ptr<PosList> position_list,
                                              std::shared_ptr<DictionaryColumn<T>> dc, const ChunkID chunk_id) {
  ...
  if (!all_false) {
    const auto& accessor = [&](size_t chunk_offset) { return id_values->get(chunk_offset); };
...
template <typename T>
void TableScan::TableScanImpl<T>::_scanColumn(std::shared_ptr<PosList> position_list,
                                              std::shared_ptr<ReferenceColumn> rc,
